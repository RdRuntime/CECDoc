...
abstract class DialogueFormulaireDocumentAbstrait extends JDialog {
    ...
    private static final DateTimeFormatter FORMAT_DATE_SAISIE = DateTimeFormatter.ofPattern("dd/MM/uuuu", Locale.ROOT);
    ...
    private boolean erreursAffichees;
    ...
    protected DialogueFormulaireDocumentAbstrait(JFrame proprietaire, TokensTheme theme, List<Image> iconesApplication, Path dossierSortieParDefaut, String titreFenetre, String titreFormulaire, String libelleBoutonGenerer) {
        ...
        int masqueRaccourci;
        try {
            masqueRaccourci = Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx();
        } catch (HeadlessException ex) {
            masqueRaccourci = KeyEvent.CTRL_DOWN_MASK;
        }
        ...
        carteEntrees.put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, masqueRaccourci), "generer-document");
        carteEntrees.put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, KeyEvent.CTRL_DOWN_MASK), "generer-document");
        carteEntrees.put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, KeyEvent.META_DOWN_MASK), "generer-document");
        carteActions.put("generer-document", new AbstractAction() {
            @Override
            public void actionPerformed(java.awt.event.ActionEvent e) {
                if (tacheActive == null && boutonGenerer.isEnabled()) {
                    lancerGeneration();
                }
            }
        });
    ...
    protected final void effacerErreursValidation() {
        if (!erreursAffichees && messagesErreurs.isEmpty()) {
            return;
        }
    ...
    protected final boolean formatPrenomValide(String prenom) {
        ...
        String texte = prenom.trim();
        if (texte.isEmpty()) {
            return false;
        }
        if (texte.contains(",")) {
            return formatPrenomsMultiplesValide(texte);
        }
        return MOTIF_PRENOM.matcher(texte).matches();
    }
    ...
    protected final boolean formatPrenomsMultiplesValide(String prenoms) {
        ...
        for (String element : elements) {
            String valeur = element == null ? "" : element.trim();
            if (valeur.isEmpty()) {
                return false;
            }
            if (!MOTIF_PRENOM.matcher(valeur).matches()) {
                return false;
            }
        }
        return true;
    ...
    protected final String dateDuJourTexte() {
        return LocalDate.now().format(FORMAT_DATE_SAISIE);
    }
    ...
    protected final SelecteurDateDialogue creerSelecteurDate() {
        ...
        return new SelecteurDateDialogue(anneeCourante, 1900, anneeCourante);
    }
    ...
    protected final class SelecteurDateDialogue extends JPanel {
        ...
        void appliquerDateTexte(String texteDate) {
            if (ParseursDate.dateSaisieNonFuture(texteDate)) {
                LocalDate date = ParseursDate.parserDateSaisie(texteDate.trim());
                definirSelectionCombo(comboAnnee, Integer.toString(date.getYear()), Integer.toString(LocalDate.now().getYear()));
                definirSelectionCombo(comboMois, Integer.toString(date.getMonthValue()), "12");
                ajusterJour();
                definirSelectionCombo(comboJour, Integer.toString(date.getDayOfMonth()), "13");
                return;
            }
        ...
        boolean estDateValide() {
            return ParseursDate.dateSaisieNonFuture(texteDate());
        }
        ...
        private void definirSelectionCombo(JComboBox<String> combo, String valeur, String valeurSecours) {
            if (comboContient(combo, valeur)) {
                combo.setSelectedItem(valeur);
                return;
            }
            if (comboContient(combo, valeurSecours)) {
                combo.setSelectedItem(valeurSecours);
            }
        }

        private boolean comboContient(JComboBox<String> combo, String valeur) {
            if (valeur == null || valeur.isBlank()) {
                return false;
            }
            for (int i = 0; i < combo.getItemCount(); i++) {
                if (Objects.equals(combo.getItemAt(i), valeur)) {
                    return true;
                }
            }
            return false;
