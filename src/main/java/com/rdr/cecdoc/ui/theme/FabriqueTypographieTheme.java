...
public final class FabriqueTypographieTheme {
    ...
    private static final int STYLES_VALIDES = Font.PLAIN | Font.BOLD | Font.ITALIC;
    ...
    private static Font pick(int style, int size, String... preferredFamilies) {
        int styleValide = normaliserStyle(style);
        for (String family : preferredFamilies) {
            if (family != null && isAvailable(family)) {
                return new Font(family, styleValide, size);
            }
        }
        return new Font("SansSerif", styleValide, size);
    }
    ...
    private static int normaliserStyle(int style) {
        int styleNormalise = style & STYLES_VALIDES;
        return styleNormalise == 0 ? Font.PLAIN : styleNormalise;
    }
}
